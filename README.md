...
The financial model has been rebuilt around a suite-tier architecture. Business-assumption corrections and founder decisions are documented in:

- `docs/founder-decision-sheet.md`
- `docs/business-contradictions.md`
- `docs/post-approval-audit-status.md`

Canonical business values are consolidated in `content/pitch-data.json` (ADR-004), and a pre-commit guard prevents the rendered JSX from drifting away from that file. Build, lint, and unit tests run green on every push and pull request via CI.
...
The project tracks two separate governance records:

- **Business-assumption corrections** are documented in `docs/business-contradictions.md`, the contradiction source of truth for this repository.
- **Founder decisions and sign-off record** are documented in `docs/founder-decision-sheet.md`.

The financial model has been rebuilt around a suite-tier architecture and the rebuild is documented in `docs/post-approval-audit-status.md`.
...
├── founder-decision-sheet.md   — founder decisions and sign-off record
└── post-approval-audit-status.md — suite-tier rebuild documentation
