...
The financial model is built around a suite-tier architecture. Canonical business values are consolidated in `content/pitch-data.json` (ADR-004), and a pre-commit guard prevents the rendered JSX from drifting away from that file. Build, lint, and unit tests run green on every push and pull request via CI.
...
The financial model is built around a suite-tier architecture, and the rendered JSX is kept in lockstep with `content/pitch-data.json` by the `verify-content-consistency` guard.
...
Run lint, tests, build, theme-sync, and content-consistency in sequence (use before committing):
...
This runs lint, tests, build, theme-sync, and content-consistency in sequence.

## Generated docs

This public demo repo does not publish generated internal documentation.

Public-facing claims are maintained through the current demo, financial model, and product roadmap.

## Pre-commit hooks

A versioned hook at `.githooks/pre-commit` runs one guard before every commit:

- `npm run verify-content-consistency` — every canonical business value rendered in JSX or Python references `content/pitch-data.json` (ADR-004).
...
Any commit that breaks the guard is blocked with a clear error pointing at the offending file and line.
